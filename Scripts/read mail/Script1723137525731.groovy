...
final String username = "aditya1993"
...
try {
    ...
    if (messages.length > 0) {
        for (Message message : messages) {
            ...
            if (email.equalsIgnoreCase("aditya_T")) {
                ...
                def matcher = textContent =~ /Test Result: (\d+)/
                if (matcher.find()) {
                    String testResult = matcher.group(0) 
                    String resultValue = matcher.group(1) 

                    println("Extracted Test Result: " + testResult)
                    println("Extracted Value: " + resultValue)
                } else {
                    println("Test Result not found in the email content.")
                }
